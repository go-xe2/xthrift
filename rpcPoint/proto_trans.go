...
func protocolListTransform(in thrift.TProtocol, out thrift.TProtocol) error {
	...
	for i := 0; i < size; i++ {
		...
		case thrift.SET:
			...
			break
		case thrift.MAP:
			if err := protocolMapTransform(in, out); err != nil {
				return err
			}
			break
		case thrift.STRUCT:
...
func protocolMapTransform(in thrift.TProtocol, out thrift.TProtocol) error {
	kt, vt, size, err := in.ReadMapBegin()
	if err != nil {
		return err
	}
	if err := out.WriteMapBegin(kt, vt, size); err != nil {
		return err
	}
	for i := 0; i < size; i++ {
		// 转换键名
	...
	if err := in.ReadMapEnd(); err != nil {
		return err
	}
	if err := out.WriteMapEnd(); err != nil {
		return err
	}
	return nil
}
